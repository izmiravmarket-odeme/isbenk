/* Modified by Arda V. for sync Jquery 3.6.0 version removeAttr, focus, isFunction, click and mouse attributes has been modified
...
&lt;div&gt;&lt;/div&gt;</pre>
  ...
  return ch.toUpperCase();
		...
		_optionsChanged: function(elem, inst, options) {
			...
			elem[inst.options.keypadOnly ? 'attr' : 'prop']('readonly', true).
		...
		_preDestroy: function(elem, inst) {
			...
			elem.empty().off('.' + inst.name)
				[inst.saveReadonly ? 'attr' : 'prop']('readonly', true);
		...
		show: function(elem) {
			...
			if (inst._input[0].type !== 'hidden') {
                inst._input[0].trigger('focus');//focus();
			}
		...
		_generateHTML: function(inst) {
			...
			html.find('button').on('mousedown', function() { $(this).addClass(activeClasses); }).
